...
promptlen = 0 # We estimate text generated from a random single token 
...
def estimate_temperature_fast(logits, selected_indices):
    ...
    Args:
        logits (torch.Tensor): Shape (N, V). On GPU.
        selected_indices (torch.Tensor): Shape (N,). On GPU.
    """
    ...
    def gradient_function(beta):
        # Handle edge case where solver tries beta <= 0 (unphysical)
...
def compute_logits_from_text(model, tokenizer, text):
    ...
    # 2. Forward Pass (No generation, just evaluation)
    with torch.no_grad():
...
for model_id in model_ids:
    ...
    bnb_config = BitsAndBytesConfig(
        ...
        bnb_4bit_compute_dtype=torch.bfloat16 # Use torch.float16 if bfloat16 is not available
    )
    ...
    del model
    del tokenizer
    torch.cuda.empty_cache() # Clear unused cached memory by PyTorch
